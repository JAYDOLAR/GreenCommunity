...
const Dashboard = () => {
  ...
  return (
    <div className="p-4 sm:p-8 pb-8 sm:pb-12 space-y-6 sm:space-y-8 bg-gradient-to-br from-background via-accent/5 to-primary/5 min-h-screen relative">
      {/* Enhanced Header */}
            ...
            <CardContent>
              <div className="space-y-2 sm:space-y-3">
                <div className="text-2xl sm:text-3xl font-bold text-foreground">
                  ...
                  <span className="text-sm sm:text-base font-normal text-muted-foreground ml-1">tons CO₂</span>
                </div>
                <div className="flex items-center gap-2">
                  <TrendingDown className="h-4 w-4 sm:h-5 sm:w-5 text-success animate-bounce" />
                  <span className="text-xs sm:text-sm text-success font-semibold">12% below target</span>
                </div>
                <ProfessionalProgress value={80} className="mt-2 sm:mt-3" />
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent>
              <div className="space-y-2 sm:space-y-3">
                <div className="text-2xl sm:text-3xl font-bold text-foreground">
                  ...
                  <span className="text-sm sm:text-base font-normal text-muted-foreground ml-1">kg CO₂</span>
                </div>
                <div className="flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-warning animate-bounce" />
                  <span className="text-xs sm:text-sm text-warning font-semibold">5% above last week</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-2 sm:space-y-3">
                <div className="text-2xl sm:text-3xl font-bold text-foreground">
                  ...
                  <span className="text-sm sm:text-base font-normal text-muted-foreground ml-1">tons</span>
                </div>
                <Badge variant="secondary" className="bg-success/15 text-success border-success/30 font-semibold animate-pulse-eco text-xs sm:text-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                  {footprintBreakdown.map((item, index) => {
                    ...
                    return (
                      <div
                        key={item.category}
                        className="flex items-center justify-between p-3 sm:p-4 rounded-xl border border-border/30 hover:border-primary/20 hover:bg-primary/5 transition-all duration-300 group animate-fade-in"
                        style={{ animationDelay: `${0.6 + (index * 0.1)}s` }}
                      >
                        <div className="flex items-center gap-3 sm:gap-4">
                          <div className={`p-2 sm:p-3 rounded-xl bg-gradient-to-br from-accent/20 to-accent/30 ${item.color} group-hover:scale-110 transition-transform duration-300`}>
                            <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
                          </div>
                          <div>
                            <div className="font-semibold text-foreground text-base sm:text-lg">{item.category}</div>
                            <div className="text-xs sm:text-sm text-muted-foreground">
              ...
              <CardContent>
                <div className="space-y-3 sm:space-y-4">
                  {recentActivities.map((activity, index) => (
                    <div 
                      key={index} 
                      className="flex items-center justify-between p-3 sm:p-4 rounded-xl border border-border/30 hover:bg-accent/20 hover:shadow-lg transition-all duration-300 animate-fade-in hover-lift group"
                      style={{ animationDelay: `${0.7 + (index * 0.1)}s` }}
                    >
                      <div className="flex-1">
                        <div className="font-semibold text-foreground text-base sm:text-lg group-hover:text-primary transition-colors">
              ...
              <CardContent className="space-y-4">
                {achievements.map((achievement, index) => {
                  ...
                  return (
                    <div 
                      key={index} 
                      className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl border-2 transition-all duration-500 hover-lift ${
                        achievement.earned 
                    ...
                    >
                      ...
                      {achievement.earned && (
                        <Badge className="bg-success/20 text-success border-success/30 font-bold animate-pulse-eco text-xs">
                          ✓ Earned
                        </Badge>
                      )}
                    </div>
          ...
          <div className="animate-slide-up" style={{ animationDelay: '0.7s' }}>
            <Card className="card-premium hover-glow">
              ...
              <CardContent className="space-y-3 sm:space-y-4">
                <Button className="w-full justify-start h-12 sm:h-14 text-base sm:text-lg font-semibold btn-professional group">
                  <Plus className="h-4 w-4 sm:h-5 sm:w-5 mr-2 sm:mr-3 group-hover:scale-110 transition-transform" />
                  Log Activity
                  <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4 ml-auto group-hover:translate-x-1 transition-transform" />
                </Button>
                <Button className="w-full justify-start h-12 sm:h-14 text-base sm:text-lg font-semibold btn-professional group">
                  <Zap className="h-4 w-4 sm:h-5 sm:w-5 mr-2 sm:mr-3 group-hover:scale-110 transition-transform" />
                  Offset Emissions
                  <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4 ml-auto group-hover:translate-x-1 transition-transform" />
                </Button>
                <Button className="w-full justify-start h-12 sm:h-14 text-base sm:text-lg font-semibold btn-professional group">
